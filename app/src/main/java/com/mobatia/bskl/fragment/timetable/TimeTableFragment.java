...
import com.mobatia.bskl.fragment.timetable.adapter.TimeTableAllWeekSelectionAdapter;
...
public class TimeTableFragment extends Fragment implements
    ...
    public TimeTableFragment(String title, String tabId) {
        this.mTitle = title;
        this.mTabId = tabId;
    }
                ...
                new RecyclerItemListener.RecyclerTouchListener() {
                    public void onClickItem(View v, int position) {
                        ...
                        if (position != 0) {
                            ...
                            {
                                TimeTableAllWeekSelectionAdapter mRecyclerAllAdapter = new TimeTableAllWeekSelectionAdapter(mContext, mPeriodModel,timeTableAllRecycler,tipContainer);
                        ...
                        if (status_code.equalsIgnoreCase("303")) {
                            ...
                            if (period.length() > 0) {
                                ...
                                for (int x = 0; x < period.length(); x++) {
                                    ...
                                    xmodel.setPeriod_id(listObject.optString("period_id"));
                                    if (!(listObject.optString("period_id").equalsIgnoreCase("")))
                                    {
                                        lun=lun+1;
                                    ...
                                    if (data.length() > 0) {
                                        for (int j = 0; j < data.length(); j++) {
                                            ...
                                            } else if (dataObject.optString("day").equalsIgnoreCase("Tuesday")) {
                                                    xmodel.setTuesday(dataObject.optString("subject_name"));

                                                xmodel.setCountT(t);

                                            } else if (dataObject.optString("day").equalsIgnoreCase("Wednesday")) {
                                                    xmodel.setWednesday(dataObject.optString("subject_name"));

                                                xmodel.setCountW(w);

                                            } else if (dataObject.optString("day").equalsIgnoreCase("Thursday")) {
                                                    xmodel.setThursday(dataObject.optString("subject_name"));

                                                xmodel.setCountTh(th);

                                            } else if (dataObject.optString("day").equalsIgnoreCase("Friday")) {
